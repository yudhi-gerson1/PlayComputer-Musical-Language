...
function buildSchedule(compiled){
  ...
  function emitPartEvents(part, baseStart, toneFactor){
    ...
    const gate = (ARTICULATION_GATE && ARTICULATION_GATE[part.articul] !== undefined)
      ? ARTICULATION_GATE[part.articul] 
      : 0.9;
    const speed = part.speedFactor || 1; // >1 = mais rápido
    let t = baseStart;

    part.events.forEach(ev => {
      try {
        const advanceSeconds = (ev.seconds || 0) / speed;
        if(!ev.pause){
          const soundSeconds = advanceSeconds * gate;
          const common = {
            time: t, 
            duration: soundSeconds, 
            channelId: part.id,
            instrument: part.instrument, 
            effectType: part.type,
            volume: part.volume !== undefined ? part.volume : 1, 
            fadeIn: part.fadeIn, 
            fadeOut: part.fadeOut,
            totalChannelSpan: null
          };

          if(part.instrument === 'drum'){
            events.push({
              type: 'drum', 
              drumType: ev.degree || ev.note || 1, 
              ...common
            });
          } else if(ev.type === 'note'){
            events.push({
              type: 'note', 
              freq: ev.freq * toneFactor, 
              ...common
            });
          } else if(ev.type === 'chord'){
            events.push({
              type: 'chord', 
              freqs: ev.freqs.map(f => f * toneFactor), 
              ...common
            });
          }
        }
  ...
  parts.forEach(part => {
    if(part.autoplay){
      const speed = part.speedFactor || 1;
      // Duração ajustada pelo speedFactor para não gerar buracos entre os repeats
      const rawDur = typeof naturalDuration === 'function' ? naturalDuration(part) : 0;
      const realDur = (rawDur / speed) || 0.001;

      for(let r = 0; r < (part.repeatCount || 1); r++){
        const factor = Math.pow(2, ((part.toneStep || 0) * r) / 12);
        emitPartEvents(part, part.startOffsetSeconds + (r * realDur), factor);
      }
    }
  });

  groupInstances.forEach(gi => {
    if(gi && gi.part){
      emitPartEvents(gi.part, gi.startOffsetSeconds, gi.toneFactor);
    }
  });
  ...
  // Marca, por canal, o intervalo total ocupado para fade proporcional
  const spanByChannel = {};
  events.forEach(e => {
    if(!e.channelId) return;
    const start = e.time;
    const end = e.time + e.duration;
    if(!spanByChannel[e.channelId]){
      spanByChannel[e.channelId] = { start, end };
    } else {
  ...
  events.forEach(e => {
    if(e.channelId && spanByChannel[e.channelId]){
      e.totalChannelSpan = spanByChannel[e.channelId];
    }
  });
  ...
  // Ordena a timeline em ordem cronológica estrita
