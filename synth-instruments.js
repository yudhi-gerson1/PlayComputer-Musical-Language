...
// Corda dedilhada com filtro de brilho em varredura
...
function scheduleOrgan(ctx, dest, freq, time, dur){
  ...
  partials.forEach((p, idx) => {
    [-3, 3].forEach(cents => {
      ...
      const og = ctx.createGain(); og.gain.value = amps[idx] * 0.25;
...
function scheduleAccordeon(ctx, dest, freq, time, dur){
  ...
  // LFO compartilhado para evitar vazamento de nós
...
function scheduleSynthBass(ctx, dest, freq, time, dur){
  ...
  const filt = ctx.createBiquadFilter(); filt.type = 'lowpass'; filt.frequency.value = Math.min(Math.max(f * 4, 150), 2000);
...
function scheduleViolin(ctx, dest, freq, time, dur){
  ...
  const g = ctx.createGain();
  g.gain.setValueAtTime(0.0001, time);
  g.gain.linearRampToValueAtTime(0.3, time + 0.06);
  ...
  lfoGain.gain.linearRampToValueAtTime(6, time + 0.15);
...
function scheduleSaxophone(ctx, dest, freq, time, dur){
  ...
  const breathGain = ctx.createGain(); breathGain.gain.value = 0.02;
...
function scheduleTrumpet(ctx, dest, freq, time, dur){
  ...
  buzzGain.gain.setValueAtTime(0.08, time);
  buzzGain.gain.exponentialRampToValueAtTime(0.001, time + 0.06);
  buzz.connect(buzzGain); buzzGain.connect(filt);
...
function scheduleChoir(ctx, dest, freq, time, dur){
  ...
  [-9, -3, 0, 3, 9].forEach(cents => {
    ...
    osc.frequency.setValueAtTime(freq, time);
    osc.detune.setValueAtTime(cents, time);
    const og = ctx.createGain(); og.gain.value = 0.06; // Ganho atenuado para evitar distorção
  ...
  try{
    ...
    switch(instrument){
      ...
      case 'guitarBass':
        ...
        break;
      case 'ukulele':
        // Corrigido: frequência real mantida sem transposição incorreta de 1.5x
        pluckedTone(ctx, dest, freq, time, dur * 0.7, { harmonics:[1,2,3,4,5], ampWeights:[1,0.45,0.24,0.14,0.07], spread:2.8, decay:Math.max(dur*0.55,0.28), attack:0.002, gain:0.4, inharm:0, filterStart:freq*20, filterEnd:freq*5, filterQ:0.6 });
        break;
